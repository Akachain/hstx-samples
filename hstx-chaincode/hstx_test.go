...
import (
	...
	"gitlab.com/akachain/highsecuretransaction/models"
)
...
func checkGetALL(t *testing.T, stub *shim.MockStub, args [][]byte) []byte {
	res := stub.MockInvoke("1", args)
	if res.Status != shim.OK {
		t.FailNow()
		return res.Payload
...
func TestMainchain(t *testing.T) {
	...
	rs7 := checkInvoke(t, stub, [][]byte{[]byte("CreateAdmin"), []byte("Admin1"), []byte("pulic key")})

	//rs3 := checkByID(t, stub, [][]byte{[]byte("GetAdminByID1"), []byte("aa"), []byte(models.ADMINTABLE)})

	//rs4 := checkGetALL(t, stub, "GetAllProposal")

	rs6 := checkGetALL(t, stub, [][]byte{[]byte("GetAllData"), []byte(models.ADMINTABLE)})
